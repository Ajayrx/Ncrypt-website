    ...
    <Card className="w-full max-w-md mx-auto overflow-hidden bg-background/50 backdrop-blur-lg border border-border/50">
      <CardContent className="p-6">
        ...
        <div className="flex flex-col items-center justify-center w-full">
          <div className={`w-48 h-48 rounded-full border-2 ${scanning ? 'border-primary nfc-pulse' : 'border-muted-foreground/30'} flex items-center justify-center mb-6`}>
            <div className={`w-32 h-32 rounded-full ${scanning ? 'bg-primary/10' : 'bg-muted/30'} flex items-center justify-center`}>
              <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className={`${scanning ? 'text-primary animate-pulse-slow' : 'text-muted-foreground/70'}`}>
